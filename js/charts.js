// Banner de alerta parpadeante: casos En Proceso entre 60%-69%
function buildAlertBanner(data) {
  const alertas = data.filter(r =>
    (r.Estado === 'En Proceso' || r.Estado === 'En proceso') &&
    r.pct >= 60 && r.pct < 70
  );
...
function buildKPIs(data) {
  ...
  document.getElementById('kpi-grid').innerHTML = `
    <div class="kpi normal">
      ...
      <div class="kpi-label">Normal — 0 a 59%</div><div class="kpi-sub">Sin alerta</div>
    </div>
    <div class="kpi atencion">
      <div class="kpi-num">${data.filter(r=>r.pct>=60&&r.pct<70).length}</div>
      <div class="kpi-label">Atención — 60 a 69%</div><div class="kpi-sub">Próximos a vencer</div>
    </div>
    <div class="kpi critico">
      <div class="kpi-num"><span class="adot o"></span>${data.filter(r=>r.pct>=70&&r.pct<100).length}</div>
      <div class="kpi-label">Crítico — 70 a 99%</div><div class="kpi-sub">Notificar gerentes</div>
    </div>
    <div class="kpi vencido">
      <div class="kpi-num"><span class="adot r"></span>${data.filter(r=>r.pct>=100).length}</div>
      <div class="kpi-label">Vencidos — ≥100%</div><div class="kpi-sub">Acción inmediata</div>
    </div>`;
}

// Gráficas de barras apiladas y top responsables
function buildCharts(data) {
  ...
  if (top.length > 0) {
    chartResp = new Chart(document.getElementById('chart-resp'), {
      type:'bar',
      data:{
        labels:top.map(r=>{ const p=r[0].split(' '); return p[0]+' '+(p[p.length-1]||''); }),
